    ...
    iotAgentLib.update(device.name, device.type, '', values, device, function(error) {
        if (error) {
            config.getLogger().error(context,
                'MEASURES-002: Couldn\'t send the updated values to the Context Broker due to an error: %s', JSON.stringify(error));
        } else {
...
/**
 * Handles an incoming MQTT message payload and extracting a logical device ID if exists.
 *
 * @param {Object} messageInfo  MQTT message body and physical device ID.
 */
function handleLogicalDevice(messageInfo) {
    var deviceId = messageInfo.deviceId;
    var messageBody = messageInfo.messageBody;

    if (messageBody.hasOwnProperty("lid") == false) {
        return deviceId;
    }
    
    var logicalDeviceId = messageBody["lid"].toString();
    delete messageBody["lid"];

    return deviceId + ".L" + logicalDeviceId;
}
...
function messageHandler(topic, message) {
    var topicInformation = topic.split('/'),
        ...
        rawDeviceId = topicInformation[2],
    ...
    var deviceId = handleLogicalDevice(messageInfo);
    ...
    iotAgentLib.alarms.release(constants.MQTTB_ALARM);
    iotaUtils.retrieveDevice(deviceId, apiKey, 'MQTT', processDeviceMeasure);
}
