...
/**
 * Handles an outcoming device ID and appends logical device ID into MQTT message payload if a logical device suffix exists.
 *
 * @param {Object} messageInfo  MQTT message body and complex device ID in format "<PHYSICAL_DEVICE_ID>[.L<LOGICAL_DEVICE_ID>]".
 */
function handleLogicalDevice(messageInfo) {
    var deviceId = messageInfo.deviceId;
    var messageBody = messageInfo.messageBody;
    ...
    var logicalDeviceId = deviceId.substring(logicalDeviceSuffixPosition + 2);

    messageBody["lid"] = logicalDeviceId;
}
...
function generateCommandExecution(apiKey, device, attribute) {
    ...
    handleLogicalDevice(commandInfo);

    payload[attribute.name] = attribute.value;
    serialized = JSON.stringify(payload);

    config.getLogger().debug(context, 'Sending command execution to device [%s] with apikey [%s] and payload [%j] ',
        device.id, apiKey, attribute);
...
function commandHandler(id, type, service, subservice, attributes, callback) {
    ...
    iotAgentLib.getDeviceByName(id, service, subservice, function(error, device) {
        if (error) {
            ...
            iotaUtils.getEffectiveApiKey(device.service, device.subservice, function(error, apiKey) {
                if (error) {
                    callback(error);
                } else {
